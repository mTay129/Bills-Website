...
def signup_step1():
    ...
    return render_template('signup1.html') # Uses updated HTML with accessibility and theme toggle
...
def signup_step2():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            cur = conn.cursor()
            cur.execute("""
                INSERT INTO private_info (public_info_id, full_name, email, phone, billing_address)
                VALUES (%s, %s, %s, %s, %s);
            """, (public_id, name, email, phone, address))
            conn.commit()
            cur.close()
            conn.close()
    ...
    return render_template('signup2.html', public_id=public_id) # Uses updated HTML with accessibility and theme toggle
